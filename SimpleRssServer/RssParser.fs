...
let createErrorFeed errorType =
    ...
    | UriHostNameMustContainDot u ->
        feedItem.Description <- $"Invalid URI: {InvalidUri.value u}. Host name must contain a dot."
        feedItem.Link <- InvalidUri.value u
    | DomainMessage.UriFormatException(u, ex) ->
        feedItem.Description <- $"Invalid URI format: {InvalidUri.value u}. {ex.GetType().Name}: {ex.Message}"
        feedItem.Link <- InvalidUri.value u
    | HttpRequestFailed(uri, waitTime) ->
        feedItem.Description <- $"Previous request(s) to {uri} failed. You can retry in {waitTime.TotalHours:F1} hours."
        feedItem.Link <- uri.AbsoluteUri
    | InvalidRssFeedFormat ex -> feedItem.Description <- $"Invalid RSS feed format. {ex.GetType().Name}: {ex.Message}"
    | HttpRequestTimedOut(uri, waitTime) ->
        feedItem.Description <- $"Request to {uri} timed out after {waitTime.TotalHours:F1} seconds."
        feedItem.Link <- uri.AbsoluteUri
    | HttpException(uri, ex) ->
        feedItem.Description <- $"Failed to get {uri}. {ex.GetType().Name}: {ex.Message}"
        feedItem.Link <- uri.AbsoluteUri
    | HttpRequestNonSuccessStatus(uri, status) ->
        feedItem.Description <- $"Failed to get {uri}. Error: {status}."
        feedItem.Link <- uri.AbsoluteUri
